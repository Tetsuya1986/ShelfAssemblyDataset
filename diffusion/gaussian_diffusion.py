...
class GaussianDiffusion:
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, dataset=None):
        ...
        # --- History/Future Splitting for Conditional Prediction ---
        is_prediction = getattr(dataset, 'task', 'generation') == 'prediction'
        if is_prediction and dataset is not None:
            history_len = int(dataset.input_seconds * dataset.opt.fps)
            model_kwargs['y']['history'] = x_start[..., :history_len]
            x_start_target = x_start[..., history_len:]
            mask = mask[..., history_len:]
        ...
        if noise is None:
            noise = th.randn_like(x_start_target)
        x_t = self.q_sample(x_start_target, t, noise=noise)
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            target = {
                ModelMeanType.PREVIOUS_X: self.q_posterior_mean_variance(
                    x_start=x_start_target, x_t=x_t, t=t
                )[0],
                ModelMeanType.START_X: x_start_target,
                ModelMeanType.EPSILON: noise,
            }[self.model_mean_type]
            assert model_output.shape == target.shape == x_start_target.shape  # [bs, njoints, nfeats, nframes]
            ...
            if getattr(dataset, 'task', 'generation') == 'collab_prediction':
